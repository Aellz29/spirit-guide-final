...
function loadCheckout() {
    ...
    cart.forEach((item) => {
        ...
        html += `
            <div class="flex items-start gap-4 border-b border-gray-100 pb-4 last:border-0 p-2">
                <div class="w-12 h-12 bg-gray-100 rounded-lg overflow-hidden shrink-0"><img src="${item.img}" class="w-full h-full object-cover"></div>
                <div class="flex-1 min-w-0">
                    <h4 class="text-[11px] font-bold uppercase text-gray-800 line-clamp-1">${item.title}</h4>
                    <p class="text-[10px] text-gray-500">${item.qty} x Rp ${price.toLocaleString('id-ID')}</p>
                </div>
                <div class="text-right"><p class="text-[11px] font-bold text-gray-900">Rp ${(price * item.qty).toLocaleString('id-ID')}</p></div>
            </div>`;
    });
